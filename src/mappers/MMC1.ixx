...
export class MMC1 : public BaseMapper
{
public:
	MMC1(std::vector<u8> chr_prg_rom, MapperProperties properties) :
		...
		if (addr <= 0x5FFF) {
			...
			if (data & 0x80) {
				...
				if (++times_written_to_control_register == 5) {
					...
					default: std::unreachable(); break; // impossible
					}
					shift_reg = 0x10;
					times_written_to_control_register = 0;
				}
...
protected:
	...
	uint chr_bank_0 : 5 = 0;
	uint chr_bank_1 : 5 = 0;
	uint chr_mirroring : 2 = 0;
	uint prg_bank : 4 = 0;
	uint prg_ram_bank : 2 = 0;
	uint prg_rom_bank_mode : 2 = 3; /* nesdev seem to suggest that many carts start in PRG ROM bank mode 3. */
	uint shift_reg : 5 = 0x10;
	uint times_written_to_control_register = 0;
